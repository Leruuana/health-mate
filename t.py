...
def callback_query(callback):
    ...
    #Тут показываются симптомы
        ...
        bot.send_message(callback.message.chat.id, text= simptom[simptom['simptom'].str.strip() == callback.data].iloc[:, [1, 2]].to_string(header=False, index=False, na_rep='').replace('\n', '<br>'), parse_mode='HTML')
        ...
        #bot.send_message(callback.message.chat.id, text=text, parse_mode='HTML')
